...
- 不要自动执行安装/构建/测试命令（例如 `pnpm install`、`pnpm build`、`npm test`、`yarn build`、`node scripts/*.js`）。
- 当前环境不支持基于 `package.json` 的依赖管理流程；默认不采用需要构建链的方案。
  ...
  "capabilities": [
    {
      ...
      "description": "Read selected file content and show preview in UI.",
      "targets": ["file"]
    },
    {
...
- `capabilities`: 可选能力声明，支持 `file/folder/any` 目标类型
- `env`: 运行时环境变量（字符串键值）
...
- 事件与状态集中管理，减少全局变量污染
- 当前宿主不支持 `package.json` 管理，默认不要引入必须构建后才能运行的前端工程
...
1. 在 Tool Hub 之外的环境完成依赖安装与构建
2. 仅拷贝最终静态产物到 `ui/`
3. `app.json` 继续使用 `uiPath: "ui/index.html"`
4. AI 在本项目内不要执行任何安装/构建/测试命令
...
- `window.toolHubAppApi.getRuntimeInfo()` -> `{ appId, launchContext, launchContextUpdatedAt }`
- `window.toolHubAppApi.subscribeLaunchContext(callback)` -> `unsubscribe()`，payload 形如 `{ launchContext, launchContextUpdatedAt }`
- `window.toolHubAppApi.notify(title, body, options?)` -> `Promise<boolean>`，触发宿主系统通知
- `window.toolHubAppApi.pickDirectory(options?)` -> `Promise<null | { path, entries?, truncated? }>`，弹目录选择框并返回绝对路径（可选目录条目）
...
- `title?: string`
- `includeEntries?: boolean`（默认 `false`）
- `maxEntries?: number`（默认 `2000`）
- `maxDepth?: number`（默认 `6`）
- `includeHidden?: boolean`（默认 `false`）

`launchContext` 协议说明（建议重点遵循）：
...
```ts
type LaunchContext = {
  source: "explorer-context-menu" | "quick-launcher" | "manual";
  requestId: string;
  requestedAt: number; // epoch ms
  payload?: string; // quick-launcher/manual 时可携带文本 payload
  capability?: {
    id: string;
    name: string;
    targets: Array<"file" | "folder" | "any">;
  };
  targets?: Array<{
    path: string;
...
- 搜索/Quick Launcher 调起：`launchContext.source === "quick-launcher"`，可用 `launchContext.payload`。
- 手动 Open Window 调起：`launchContext.source === "manual"`。
- 资源管理器右键分发：`launchContext.source === "explorer-context-menu"`，并携带 `capability + targets`。

推荐路由逻辑（最小示例）：

```js
function handleLaunchInput(info) {
  const context = info?.launchContext;
  ...
  if (context.source === "explorer-context-menu") {
    // 能力分发流程：按 capability.id + targets 处理
    return handleCapabilityDispatch(context);
  }

  if (context.source === "quick-launcher" && typeof payload === "string" && payload.trim()) {
    // 搜索 payload 流程
    return handleSearchPayload(payload.trim());
  }

  if (context.source === "manual") {
    // 手动打开窗口流程（可按需忽略 payload）
    return handleManualOpen();
  }
...
const selected = await window.toolHubAppApi.pickDirectory({
  title: "Select project directory",
  includeEntries: true,
  maxEntries: 120,
  maxDepth: 4,
});
console.log(selected?.path, selected?.entries?.length, selected?.truncated);
```
...
console.log("launchContext:", info.launchContext);

const unsubscribeContext = window.toolHubAppApi.subscribeLaunchContext((payload) => {
  console.log("context updated:", payload.launchContext);
});

await window.toolHubAppApi.notify("Hello App", "Context received.");
...
- 模板 UI 的「Capability Demo: Read File Content」按 capability id 工作，固定读取 `handle-files`。
- 当 `launchContext?.source === "explorer-context-menu"` 且 `launchContext.capability?.id === "handle-files"` 时，会自动读取 `targets` 中的文件并展示预览。
